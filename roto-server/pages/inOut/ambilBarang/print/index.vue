<template>
  <div class="bg-gray-300 w-widthContent ml-auto overflow-x-hidden">
    <section class="bg-gray-100 min-h-screen w-widthContentField m-auto mt-7 p-4">
        ...
        <p class="text-center text-lg text-gray-700 font-semibold">Daftar ambil barang</p>
        ...
        <div id="printform" v-if="printForm.dataAwal !== '' && printForm.dataAkhir !== ''" class="mt-4 mb-5">
          <h1 class="text-center text-black font-bold" style="font-size:20px;text-align:center;">Daftar ambil barang</h1>
          <table width="100%" border="1" style="border-collapse: collapse;" class="mt-2">
              ...
              <tbody>
                  <tr v-for="(main,index) in filterData" :key="index">
                      ...
                      <td class="border border-black py-2" style="border-width:1px;border-color:black;border-collapse:collapse;text-align: center;padding-top:2px;">{{main.nama_pengambil}}</td>
          ...
          <div class="flex justify-between w-full" style="display: flex;justify-content:space-around;margin-top:10px;">
            <div class="h-20 flex flex-col justify-between text-center" style="text-align: center;">
              <p>Nama penambah</p>
              <p style="transform:translateY(20px);">{{printSign.pengambil}}</p>
            </div>
            <div class="h-20 flex flex-col justify-between text-center" style="text-align:center;">
        ...
        <table v-else class="table space-y-6 container mx-auto table-auto border-collapse mt-7 divide-y divide-gray-300 border-collapse" ref="listitem" id="listitem">
            ...
            <tbody class="text-center bg-white bg-opacity-40 divide-y divide-gray-300">
                <tr class="text-sm uppercase" v-for="(main,index) in alldataAmbilbarang" :key="index">
                    <td>{{$moment(main.tanggal).format("DD-MM-YYYY")}}</td>
                    <td class="py-3">{{main.nama_pengambil}}</td>
...
export default {
  ...
  computed:{
    filterData(){
        return this.allAmbilbarang.filter((item,index) => index <= this.allAmbilbarang.findIndex(dat => dat.tanggal == this.printForm.dataAwal) && index >= this.allAmbilbarang.findIndex(dat => dat.tanggal == this.printForm.dataAkhir))
    },
    userpa(){
  ...
  methods:{
    async getAllDataAmbilbarang(){
      const idlogin = this.$auth.user.id
        try{
            const resp = await this.$axios.get(`/inout/ambilbarang/${this.lokasi}/${idlogin}`)
            if(resp){
                resp.data.forEach(ambilbarang => {
                    this.alldataAmbilbarang.push(ambilbarang)
                })
                this.allAmbilbarang = this.alldataAmbilbarang.map(item => {
                  return {...item,tanggal:this.$moment(item.tanggal).format("YYYY-MM-DD HH:mm:ss")}
                })
                this.allAmbilbarang.map(date => {
