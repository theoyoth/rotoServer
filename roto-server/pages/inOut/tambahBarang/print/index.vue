<template>
  <div class="bg-gray-300 w-widthContent ml-auto overflow-x-hidden">
    <section class="bg-gray-100 min-h-screen w-widthContentField m-auto mt-7 p-4">
        ...
        <p class="text-center text-lg text-gray-700 font-semibold">Daftar tambah barang</p>
        ...
        <div id="printform" v-if="printForm.dataAwal !== '' && printForm.dataAkhir !== ''" class="mt-4 mb-5">
          <h1 class="text-center text-black font-bold" style="font-size:20px;text-align:center;">Daftar tambah barang</h1>
          <table width="100%" border="1" style="border-collapse: collapse;" class="mt-2">
              <thead>
                  <tr class="text-xs text-gray-200"> 
                      ...
                      <th class="border border-black py-2" style="border-width:1px;border-color:black;padding-top:2px;font-weight:bold;color:black;">nama barang</th>
              ...
              <tbody>
                  <tr v-for="(main,index) in filterData" :key="index">
                      ...
                      <td class="border border-black py-2" style="border-width:1px;border-color:black;border-collapse:collapse;text-align: center;padding-top:2px;">{{main.nama_penambah}}</td>
          ...
          <div class="flex justify-between w-full" style="display: flex;justify-content:space-around;margin-top:10px;">
            <div class="h-20 flex flex-col justify-between text-center" style="text-align: center;">
              <p>Nama penambah</p>
              <p style="transform:translateY(20px);">{{printSign.penambah}}</p>
            </div>
            <div class="h-20 flex flex-col justify-between text-center" style="text-align:center;">
        ...
        <table v-else class="table space-y-6 container mx-auto table-auto border-collapse mt-7 divide-y divide-gray-300 border-collapse" ref="listitem" id="listitem">
            <thead class="bg-white text-sm bg-gray-700">
                <tr class="text-xs text-gray-200"> 
                    ...
                    <th class="font-semibold w-24">kuantitas</th>
                    <th class="font-semibold w-1/3">kepentingan</th>
                </tr>
            </thead>
            <tbody class="text-center bg-white bg-opacity-40 divide-y divide-gray-300">
                <tr class="text-sm uppercase" v-for="(main,index) in alldataTambahbarang" :key="index">
                    <td>{{$moment(main.tanggal).format("DD-MM-YYYY")}}</td>
                    <td class="py-3">{{main.nama_penambah}}</td>
...
export default {
  ...
  computed:{
    filterData(){
        return this.allTambahbarang.filter((item,index) => index <= this.allTambahbarang.findIndex(dat => dat.tanggal == this.printForm.dataAwal) && index >= this.allTambahbarang.findIndex(dat => dat.tanggal == this.printForm.dataAkhir))
    },
    userpa(){
  ...
  methods:{
    async getAllDataTambahbarang(){
      const idlogin = this.$auth.user.id
        try{
            const resp = await this.$axios.get(`/inout/tambahbarang/${this.lokasi}/${idlogin}`)
            if(resp){
                resp.data.forEach(tambahbarang => {
                    this.alldataTambahbarang.push(tambahbarang)
                })
                this.allTambahbarang = this.alldataTambahbarang.map(item => {
                  return {...item,tanggal:this.$moment(item.tanggal).format("YYYY-MM-DD HH:mm:ss")}
                })
                this.allTambahbarang.map(date => {
