        ...
        <div>
            <div class="grid grid-cols-2">
                <div class="mb-4 has-tooltip">
                    ...
                    <div class="flex flex-col w-72">
                        <ValidationProvider rules="required|passchar" v-slot={errors}>
                            <input type="text" v-model="inputMouse.merek" name="merek" id="merek" class="p-2 w-full rounded-lg bg-gray-300 outline-none uppercase">
                            <p class="text-xs text-right mt-1 text-red-500">{{errors[0]}}</p>
                        </ValidationProvider>
                    </div>
                </div>
                <div class="mb-4 has-tooltip" :class="[inputMouse.merek !== '' ? 'incop' : 'decop']">
                     <span class="tooltip text-xs rounded shadow-lg p-1 bg-gray-700 text-white ml-32">masukan nama model CCTV</span>

                    <label for="model" class="block mb-2 text-sm">model</label>
                    <div class="flex flex-col w-72">
                        <ValidationProvider rules="required|passchar" v-slot={errors}>
                            <input type="text" v-model="inputMouse.model" name="model" id="model" class="p-2 w-full rounded-lg bg-gray-300 outline-none uppercase" :disabled="inputMouse.merek === ''">
                            <p class="text-xs text-right mt-1 text-red-500">{{errors[0]}}</p>
                        </ValidationProvider>
                    </div>
                </div>
                <div class="mb-4 has-tooltip" :class="[inputMouse.model !== '' ? 'incop' : 'decop']">
                    ...
                    <div class="flex flex-col w-72">
                        <ValidationProvider rules="required|passchar" v-slot={errors}>
                            <input type="text" v-model="inputMouse.tipe" name="tipe" id="tipe" class="p-2 w-full rounded-lg bg-gray-300 outline-none uppercase" :disabled="inputMouse.model === ''">
                            <p class="text-xs text-right mt-1 text-red-500">{{errors[0]}}</p>
                        </ValidationProvider>
                    </div>
                </div>
                 <div class="mb-4 has-tooltip" :class="[inputMouse.tipe !== '' ? 'incop' : 'decop']">
                    ...
                    <div class="flex flex-col w-72">
                        <ValidationProvider rules="required" v-slot={errors}>
                            <input type="date" v-model="inputMouse.tahun" name="tahun" id="tahun" class="p-2 w-full rounded-lg bg-gray-300 outline-none" :disabled="inputMouse.tipe === ''">
                            <p class="text-xs text-right mt-1 text-red-500">{{errors[0]}}</p>
                        </ValidationProvider>
                    </div>
                </div>
                <div class="mb-4 has-tooltip" :class="[inputMouse.tipe !== '' ? 'incop' : 'decop']">
                    ...
                    <div class="flex flex-col w-72">
                        <ValidationProvider rules="required" v-slot={errors}>
                            <input type="date" v-model="inputMouse.garansi" name="garansi" id="garansi" class="p-2 w-full rounded-lg outline-none bg-gray-300 uppercase" :disabled="inputMouse.tipe === ''">
                            <p class="text-xs text-right mt-1 text-red-500">{{errors[0]}}</p>
                        </ValidationProvider>
                    </div>
                </div>                
...
export default {
    ...
    methods:{
        async postInputMouse(){
            const res = await this.$axios.post('/master/inputmouse',{
